...
const Projects = () => {
  ...
  const showComingSoon = activeTab === "All" || activeTab === "Full Stack";
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-6 pt-28 pb-20">
        ...
        {/* Grid — auto-rows-fr makes every row the same height */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 auto-rows-fr gap-6">
          ...
          {filtered.map((project, i) => (
            ...
            >
              ...
              <div className="flex gap-4 border-t border-white/[0.06] pt-5 flex-shrink-0">
                {project.demo && (
          ...
          {/* Coming Soon Card */}
          {showComingSoon && (
            <div className="relative h-full flex flex-col border border-dashed border-violet-500/25 bg-violet-500/[0.02] rounded-2xl p-7 items-center justify-center text-center hover:border-violet-500/45 hover:-translate-y-1.5 transition-all duration-300">
              <div className="relative w-16 h-16 rounded-full bg-violet-500/10 border border-violet-500/25 flex items-center justify-center text-3xl mb-5 flex-shrink-0">
                🚀
                <span className="absolute inset-0 rounded-full border border-violet-500/15 animate-ping" />
              </div>
              <h3 className="text-xl font-semibold text-violet-300 mb-2">Coming Soon</h3>
              <p className="text-sm text-gray-600 leading-relaxed mb-5 max-w-[200px]">
                Something exciting is in the works. Stay tuned!
              </p>
              <span className="text-[11px] tracking-widest uppercase text-violet-400 bg-violet-400/10 border border-violet-400/20 px-4 py-1.5 rounded-full animate-pulse">
                In Progress
              </span>
            </div>
          )}
